...
def extract_features(video_number, videos_dir, output_dir, batch_size, num_threads, queue_size, num_gpus):
    # Defining variables
    input_size = (112, 112)
    length = 16
    wait_time = 0.1

    output_path = os.path.join(output_dir, 'video_{}_features_activitynet.hdf5'.format(video_number))
...
output = '/home/lluc/PycharmProjects/TFG/video/video_features/devset/changed_fps'
batch_size = 32
num_threads = 8
queue_size = 12
num_gpus = 1

for video_num in range(46, 52):
    print('\033[92m'+'Video number '+str(video_num)+'\033[0m')
    directory = '/home/lluc/PycharmProjects/TFG/video/data/devset/video_{}/movies'.format(video_num)
    extract_features(
        video_num,
        directory,
        output,
...
#                         help='directory where to store the extracted features (default: %(default)s)')
...
#                         default=12,
#                         help='maximum number of elements at the queue when fetching videos (default %(default)s)')
